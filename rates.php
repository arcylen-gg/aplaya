			...
			<!-- 		<div class="panel panel-default">
						<div class="panel-body">	 -->
							<fieldset>
								...
								<?php 
				  		$mydb->setQuery("SELECT *,typeName FROM room ro, roomtype rt WHERE ro.typeID = rt.typeID AND gallery_img = 0");
						...
						<input type="hidden" name="roomprice" value="'.$result->price.'"/>';
				  		echo '<p><strong>Amenities Type: '.$result->typeName.'<br/> 
		  						<strong>Max Adults: '.$result->Adults.',<br/>  Max Children: '.$result->Children.'<br/>
		  						<strong>Day/Night Rate: </strong> &#8369 '. number_format($result->price).' </p>';
		                        $room_checker = $result->typeID;  
	            			
                            $status=$rows['STATUS'];
							if($status=='pending' AND $room_checker != 96){
							  	echo '
							...
							}elseif($status=='Confirmed'AND $room_checker != 96){
								echo '<div style="margin-top:10px; color: rgba(0,0,0,1); font-size:16px;"><strong>Book!</strong></div><br>';
							}elseif($status=='Checkedin'AND $room_checker != 96){
								echo '<div style="margin-top:10px; color: rgba(0,0,0,1); font-size:16px;"><strong>RESERVED!</strong></div><br>';
							}else{
