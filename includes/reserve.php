...
class Reservation{
	...
	function single_reservation($id=0){
			global $mydb;
			$mydb->setQuery("SELECT * FROM ".self::$tbl_name." LEFT JOIN room ON reservation.roomNo = room.roomNo Where `reservation_id`= {$id} LIMIT 1  ");
...
$mydb->setQuery($sql);
	...
	public function update_archive($guest_id=0) 
	{
	  global $mydb;
		$attributes = $this->sanitized_attributes();
		$attribute_pairs = array();
